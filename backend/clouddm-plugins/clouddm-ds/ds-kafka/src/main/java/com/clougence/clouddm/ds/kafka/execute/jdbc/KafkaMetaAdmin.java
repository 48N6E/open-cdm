...
import com.clougence.clouddm.ds.kafka.execute.KafkaMetaJson;
...
/**
 * Runs on the Kafka driver ClassLoader (has kafka-clients). Invoked reflectively from plugin CL.
 */
@Slf4j
public final class KafkaMetaAdmin {

    private static final long ADMIN_TIMEOUT_MS = 30000L;

    private KafkaMetaAdmin(){
    }

    public static String getVersion(Connection connection) throws Exception {
        AdminClient admin = KafkaAdminUnwrap.unwrap(connection);
    ...
    public static void testConnect(Connection connection) throws Exception {
        AdminClient admin = KafkaAdminUnwrap.unwrap(connection);
        DescribeClusterResult cluster = admin.describeCluster();
    ...
    public static List<Value> selectTopics(Connection connection, String pattern) throws Exception {
        AdminClient admin = KafkaAdminUnwrap.unwrap(connection);
    ...
    public static List<Value> selectConsumerGroups(Connection connection, String pattern) throws Exception {
        AdminClient admin = KafkaAdminUnwrap.unwrap(connection);
    ...
    public static List<Value> selectBrokers(Connection connection, String pattern) throws Exception {
        AdminClient admin = KafkaAdminUnwrap.unwrap(connection);
        DescribeClusterResult cluster = admin.describeCluster();
        String clusterId = cluster.clusterId().get(ADMIN_TIMEOUT_MS, TimeUnit.MILLISECONDS);
    ...
    public static Value loadTopic(Connection connection, String topicName) throws Exception {
        AdminClient admin = KafkaAdminUnwrap.unwrap(connection);
    ...
    public static Value loadConsumerGroup(Connection connection, String groupId) throws Exception {
        AdminClient admin = KafkaAdminUnwrap.unwrap(connection);
    ...
    public static Value loadBroker(Connection connection, String brokerName) throws Exception {
        List<Value> brokers = selectBrokers(connection, null);
