...
public class CrdbConfigSpi extends AbstractPgConfigSpi {
    ...
    public void customizePanels(Map<DsConfigGroup, UiPanel> panels) {
        UiPanel sslPanel = panels.get(DsConfigGroup.SSH_SSL);
        if (sslPanel == null) {
            return;
        }
        UiPanelField sslMode = sslPanel.findField(DataSourceConfig.Fields.sslMode);
        if (sslMode == null) {
            return;
        }
        sslMode.setDefaultValue(strValueDef(SslMode.TRUST.name()));
    }
