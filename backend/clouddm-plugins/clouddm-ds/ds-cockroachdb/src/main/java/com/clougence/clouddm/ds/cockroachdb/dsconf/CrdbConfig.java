 ...
 */
package com.clougence.clouddm.ds.cockroachdb.dsconf;
...
@Setter
@FieldNameConstants
@Serialization(provider = CrdbSerializationSpi.PROVIDER_NAME)
...
public class CrdbConfig extends DataSourceConfig {
    ...
    @ConfigDef(name = Fields.clientTimeZone, //
            group = DsConfigGroup.OPTIONS, labelKey = CrdbConfigI18nKeys.CONFIG_RDB_CLIENT_TIME_ZONE_LABEL, descKey = CrdbConfigI18nKeys.CONFIG_RDB_CLIENT_TIME_ZONE_DESC, readOnly = false)
    private String  clientTimeZone;
    @ConfigDef(name = Fields.connectTimeoutMs, defaultValue = "5000", //
            group = DsConfigGroup.ADVANCED, labelKey = CrdbConfigI18nKeys.CONFIG_RDB_CONN_TIMEOUT_MS_LABEL, descKey = CrdbConfigI18nKeys.CONFIG_RDB_CONN_TIMEOUT_MS_DESC, readOnly = false)
    private Long    connectTimeoutMs;
    @ConfigDef(name = Fields.soTimeoutSec, defaultValue = "10", //
            group = DsConfigGroup.ADVANCED, labelKey = CrdbConfigI18nKeys.CONFIG_DS_SO_TIMEOUT_MS_LABEL, descKey = CrdbConfigI18nKeys.CONFIG_DS_SO_TIMEOUT_MS_DESC, readOnly = false)
    ...
    public Properties asDriverProperties() {
        ...
        if (this.getSslMode() != null) {
            switch (this.getSslMode()) {
                case CA -> {
                    if (!hasSslCaConfig()) {
                        throw new IllegalArgumentException("CockroachDB CA certificate is required.");
                    }
                ...
                case CLIENT_CERT -> {
                    ...
                    if (!hasSslClientCertConfig()) {
                        throw new IllegalArgumentException("CockroachDB client certificate is required.");
                    }
                    if (!hasSslClientKeyConfig()) {
                        throw new IllegalArgumentException("CockroachDB client private key is required.");
                    }
