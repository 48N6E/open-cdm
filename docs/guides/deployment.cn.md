...
```bash
# 一键启动
docker run -d --name cgdm-alone -p 8222:8222 bladepipe/cgdm-alone:3.3.0
...
services:
  dm_alone:
    image: clougence/cgdm-alone:3.3.0
...
docker run -d --name dm_sidecar \
  ...
  -e APP_SERVE_NAME=dm_console \
  -e APP_SERVE_PORT=8008 \
  bladepipe/cgdm-sidecar:3.3.0
```

中国区部署时，只需将镜像替换为：

- `cloudcanal-registry.cn-shanghai.cr.aliyuncs.com/clougence/cgdm-console:3.3.0`
- `cloudcanal-registry.cn-shanghai.cr.aliyuncs.com/clougence/cgdm-sidecar:3.3.0`
...
services:
  ...
  dm_console:
    image: clougence/cgdm-console:x86_64-3.3.0
  ...
  dm_sidecar:
    image: clougence/cgdm-sidecar:x86_64-3.3.0
